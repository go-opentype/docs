...
font blob, build a face at a pixel size, then pull per-rune advances and
...
!!! warning "Not yet implemented (roadmap)"
    - **CFF / OpenType (`OTTO`) outlines** — `Parse` returns a clear error
      for CFF/CFF2-flavoured fonts today.
    - **GPOS / GSUB shaping**, ligatures, contextual substitution — this is
      the layer [`shape`](shape.md) composes on top once a font exposes it.
    - **Kerning** (`kern` table / GPOS pairs).
    - **Hinting** — TrueType instructions are parsed but not executed;
      rasterisation is unhinted, uniform supersampling.
    - **Vertical metrics** (`vhea`/`vmtx`) and cmap formats other than 4/12.
    - **OpenType Variations** (variable fonts / `fvar`/`gvar`/`avar`) — not
      supported; see [fonts](fonts.md) for how the bundled variable-font
      upstreams are pinned to a static default instance instead.

Rasterisation favours correctness and portability over the last drop of
